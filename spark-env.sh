...
SPARK_WORKER_CORES=8 # to set the number of cores to use on this machine
SPARK_WORKER_MEMORY=16g # to set how much total memory workers have to give executors (e.g. 1000m, 2g)
# - SPARK_WORKER_PORT / SPARK_WORKER_WEBUI_PORT, to use non-default ports for the worker
# - SPARK_WORKER_DIR, to set the working directory of worker processes
# - SPARK_WORKER_OPTS, to set config properties only for the worker (e.g. "-Dx=y")
SPARK_DAEMON_MEMORY=4g #, to allocate to the master, worker and history server themselves (default: 1g).
